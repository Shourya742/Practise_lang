use std::{collections::VecDeque, io, net::{IpAddr, Ipv4Addr, SocketAddr}};

use futures::{Future, Poll, Async};
use tokio_core::{net::UdpSocket, reactor::Core};
...
/// outgoing packet queue. Incoming messages may be received and processed
/// while outgoing writes are pending. This is in contrast to the flip-flop
/// operation of echo-udp.rs where the program is either in  a sending state 
/// or receiving state at any given point in time.
...
impl Future for UdpServer {
    ...
    /// Read and write as needed, storing read packets in an outgoing queue
    /// for later writing. We avoid the try_nb! macro here (and its potential for early return) 
    /// so taht a WouldBlock on either the reading or writing doesn't prevent progress of the other.

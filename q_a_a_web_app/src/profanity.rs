...
pub async fn check_profanity(content: String) -> Result<String,handle_errors::Error> {
    ...
    let res = client.post("https://api.apilayer.com/bad_words?censor_character=*").header("apikey", "a5uBhNhUAd2DJDTQFr9rpOyN7tybSHlG").body("a list with shit words").send().await.map_err(|e| handle_errors::Error::MiddlewareReqwestAPIError(e))?;

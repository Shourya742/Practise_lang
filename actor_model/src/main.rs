use tokio::sync::{mpsc::channel, mpsc::{Receiver, Sender}, oneshot};
...
pub async fn set(key: String, value: Vec<u8>) -> Result<(), std::io::Error> {
    ...
    ROUTER_SENDER.get().unwrap().send(
        RoutingMessage::KeyValue(
            KeyValueMessage::Set(
                SetKeyValueMessage { key, value, response: tx }
            )
        )
    ).await.unwrap();
    rx.await.unwrap();
...
pub async fn get(key: String)-> Result<Option<Vec<u8>>,std::io::Error> {
    let (tx, rx) = oneshot::channel();
    ROUTER_SENDER.get().unwrap().send(
        RoutingMessage::KeyValue(KeyValueMessage::Get(GetKeyValueMessage { key, response: tx }))
    ).await.unwrap();
    Ok(rx.await.unwrap())
}
...
pub async fn delete(key: String) -> Result<(), std::io::Error> {
    let (tx, rx) =  oneshot::channel();
    ROUTER_SENDER.get().unwrap().send(
        RoutingMessage::KeyValue(KeyValueMessage::Delete(DeleteKeyValueMessage { key, response: tx }))
    ).await.unwrap();
    rx.await.unwrap();
    Ok(())
}
